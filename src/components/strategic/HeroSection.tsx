...
const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#022c22] selection:bg-[#C9A84C]/30">
      {/* Background Decor */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-[#C9A84C]/10 blur-[120px] rounded-full" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-[#C9A84C]/10 blur-[120px] rounded-full" />
      </div>
      ...
      <div className="relative z-10">
        ...
        <nav className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full overflow-hidden ring-2 ring-[#C9A84C]/50 shadow-lg shadow-[#C9A84C]/20 bg-[#022c22]">
              ...
              <div className="hidden sm:block">
                <h1 className="font-bold text-sm tracking-tight text-white">BIRD<span className="text-[#C9A84C] font-black"> 2026-2035</span></h1>
                <p className="text-[10px] text-[#64748b] font-bold tracking-widest uppercase">Strategic Plan</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="hidden md:flex items-center gap-2 px-3 py-1 bg-[#022c22]/50 rounded-full border border-[#C9A84C]/20">
                <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                <span className="text-[10px] text-[#64748b] font-bold uppercase tracking-wider">Live</span>
              </div>
              {isAuthenticated ? (
                <button onClick={onStartPlanning}
                  className="flex items-center gap-2 px-4 py-2 bg-[#022c22]/60 hover:bg-[#022c22]/40 text-white rounded-lg text-sm font-semibold transition-all border border-[#C9A84C]/20">
                  <User className="w-4 h-4 text-[#C9A84C]" /> {userName || 'Dashboard'}
                </button>
              ) : (
                <button onClick={onSignIn}
                  className="flex items-center gap-2 px-4 py-2 bg-white text-[#022c22] hover:bg-[#E8C560] rounded-lg text-sm font-bold transition-all">
        ...
        <div className="max-w-7xl mx-auto px-6 pt-16 pb-24 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#C9A84C]/10 border border-[#C9A84C]/20 text-[#C9A84C] text-xs font-bold uppercase tracking-widest mb-8">
          ...
          <h1 className="text-5xl md:text-7xl font-black text-white mb-8 tracking-tight leading-[1.1]">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E8C560] via-[#C9A84C] to-indigo-400">
              A Hub for Resilient and Ethical Growth.
            </span>
          </h1>
          <p className="text-lg text-[#64748b]/80 mb-12 max-w-2xl mx-auto leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8">
            <button onClick={onStartPlanning}
              className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-[#C9A84C] to-[#B8942E] text-white rounded-xl font-bold text-lg hover:shadow-2xl hover:shadow-[#C9A84C]/40 transition-all hover:-translate-y-1 flex items-center justify-center gap-2">
              Open My Strategic Workspace <ArrowRight className="w-5 h-5" />
            </button>
            <button onClick={onViewDemo}
              className="w-full sm:w-auto px-8 py-4 bg-[#022c22]/60 text-white rounded-xl font-bold text-lg border border-[#C9A84C]/20 hover:bg-[#022c22]/40 transition-all flex items-center justify-center gap-2">
              <Play className="w-5 h-5 text-[#C9A84C]" /> Load BIRD Sample Plan
            </button>
          </div>
          ...
          {/* BIRD Banner */}
          <div className="relative max-w-6xl mx-auto mb-16 rounded-2xl overflow-hidden border border-[#C9A84C]/15 shadow-2xl shadow-[#C9A84C]/20">
            <img src="https://lydsisparsmvextskevw.supabase.co/storage/v1/object/public/bird-images/1.Banner.png"
              alt="BIRD 2026-2035 image banner" className="w-full h-auto object-cover" loading="eager"
              onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />
          </div>
        </div>
        ...
        ═══════════════════════════════════════════════════════════════════════ */}
        <div className="max-w-7xl mx-auto px-6 py-16 border-t border-[#C9A84C]/15">
          <div className="text-center mb-12">
            ...
            <p className="text-[#64748b]/80 max-w-2xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {siteLinks.map((site, idx) => (
                <a key={idx} href={site.url} target="_blank" rel="noopener noreferrer"
                  className="group flex items-center gap-3 p-4 rounded-xl bg-[#022c22]/50 border border-[#C9A84C]/20 hover:border-[#C9A84C]/50 hover:bg-[#C9A84C]/5 transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-[#64748b] truncate">{site.description}</p>
                  </div>
                  <ChevronRight className="w-4 h-4 text-[#ecfdf5]/80 group-hover:text-[#C9A84C]" />
                </a>
              ))}
            </div>
          </div>
        ...
        {/* Features Grid */}
        <div className="max-w-7xl mx-auto px-6 py-24 border-t border-[#C9A84C]/15">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((feature, idx) => {
              const Icon = feature.icon;
              return (
                <div key={idx} className="group p-6 rounded-2xl hover:bg-[#022c22]/50 transition-all duration-300 border border-transparent hover:border-[#C9A84C]/20">
                  ...
                  <p className="text-sm text-[#64748b]/80 leading-relaxed">{feature.description}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-24 border-t border-[#C9A84C]/15">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#059669]/10 border border-emerald-500/20 text-[#6ee7b7] text-xs font-bold uppercase tracking-widest mb-4">
              <span>Our Guiding Philosophy</span>
            </div>
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              The <span className="text-[#6ee7b7]">C.A.R.E.</span> Framework & <span className="text-amber-400">Khalifa</span> Stewardship
            </h2>
            <p className="text-[#64748b]/80 max-w-3xl mx-auto text-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {[
              ...
              { letter: "A", title: "Action-Oriented", desc: "Focused on identified leverage points, binding constraints, and executable interventions.", color: "from-[#C9A84C] to-indigo-600" },
              ...
              <div key={item.letter} className="p-6 rounded-2xl bg-[#022c22]/50 border border-[#C9A84C]/20 hover:border-[#022c22]/80 transition-all">
                ...
                <p className="text-sm text-[#64748b]/80 leading-relaxed">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-6 pb-32 text-center">
          <div className="p-12 rounded-3xl bg-gradient-to-b from-[#022c22]/60 to-[#011a12] border border-[#C9A84C]/20 shadow-3xl">
            <h2 className="text-3xl font-black text-white mb-4">Ready to Drive the Bangsamoro Investment Boom?</h2>
            <p className="text-[#64748b]/80 mb-10">
            ...
            <button onClick={onStartPlanning}
              className="px-10 py-5 bg-white text-[#022c22] rounded-2xl font-black text-xl hover:bg-[#C9A84C] transition-all hover:scale-105 active:scale-95">
              Start Planning Now →
            </button>
          </div>
        </div>
        ...
        {/* Footer */}
        <footer className="border-t border-[#C9A84C]/15 py-12 bg-[#011a12]/50">
          <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex flex-col items-center md:items-start gap-2">
              ...
              <p className="text-[10px] text-[#ecfdf5]/80 font-bold uppercase tracking-[0.2em]">BOI-MTIT, BARMM © 2026 · Built by ASilva Innovations</p>
            </div>
