...
import {
  ...
  ExternalLink, X, ClipboardCheck // Added for Validation Survey
} from 'lucide-react';
...
const NAV_TARGETS = [
  ...
  { id: 'nav-export',    label: 'Plan Generator / Export', view: 'export', section: 'Navigation' },
...
const Topbar: React.FC<TopbarProps> = ({
  ...
  // Build searchable index from active plan
      ...
      <div className="flex items-center justify-between px-3 sm:px-4 lg:px-6 py-2.5 sm:py-3 gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="relative min-w-0" ref={planRef}>
            ...
            {showPlanSelector && (
              <div
                className="absolute top-full left-0 mt-2 w-72 bg-[#1E293B] rounded-2xl shadow-2xl border border-white/10 py-2 z-50 backdrop-blur-xl"
                role="listbox"
              >
                <div className="max-h-60 overflow-y-auto px-2">
                  ...
                  {plans.map((p) => (
                    <button
                      key={p.id}
                      role="option"
                      aria-selected={p.id === currentPlan?.id}
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          ...
          <div className="relative" ref={searchRef}>
            ...
            <div className="hidden md:flex items-center gap-2 px-3 py-2 bg-white/10 rounded-xl border border-white/10 focus-within:border-cyan-500/50 transition-colors">
              ...
              <input
                type="text"
                placeholder="Search plans, KPIs, modules…"
                value={searchTerm}
                onChange={(e) => { setSearchTerm(e.target.value); setSearchOpen(true); }}
                onFocus={() => setSearchOpen(true)}
                className="bg-transparent border-none outline-none text-sm w-32 lg:w-52 text-slate-100 placeholder:text-slate-500"
              ...
              <div className="md:hidden absolute right-0 top-full mt-2 w-72 bg-[#1E293B] rounded-2xl shadow-2xl border border-white/10 p-3 z-50">
                <div className="flex items-center gap-2 px-2 py-1.5 bg-white/10 rounded-lg border border-white/10">
                  ...
                  <input
                    autoFocus
                    type="text"
                    placeholder="Search BIRD platform…"
                    value={searchTerm}
          ...
          <button
            ...
            className="p-2 rounded-lg text-slate-300 hover:text-yellow-400 hover:bg-white/10 transition-all"
          ...
          {isAuthenticated ? (
            <div className="relative" ref={accountRef}>
              <button
                ...
                className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center text-white text-xs font-bold shadow-lg hover:scale-105 transition-transform border border-white/20"
              ...
              {showAccountMenu && (
                <div className="absolute right-0 top-full mt-2 w-64 bg-[#1E293B] rounded-2xl shadow-2xl border border-white/10 py-2 z-50" role="menu">
                  ...
                  {[
                    ...
                    <button key={label} role="menuitem" onClick={action} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-slate-300 hover:bg-white/10">
                  ...
                  {currentPlan && (
                    <>
                      <button role="menuitem" onClick={onShare} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-slate-300 hover:bg-white/10">
                      ...
                      {activeShareLink && (
                        <div className="mx-4 my-2 p-2 bg-slate-800 rounded-lg space-y-1">
                          ...
                          <div className="flex gap-1">
                            ...
                            <button onClick={onRevokeShare} className="flex-1 text-[10px] py-1 bg-rose-500/20 text-rose-300 rounded">
                              Revoke
                            </button>
                          </div>
                        </div>
                  ...
                  {isAdmin && (
                    <button role="menuitem" onClick={() => { onOpenAdmin(); setShowAccountMenu(false); }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-amber-400 hover:bg-white/10">
                      <ShieldAlert className="w-4 h-4" /> Admin Dashboard
                    </button>
                  )}

                  <a
                    href={EXTERNAL_URLS.USER_MANUAL}
                    target="_blank"
                    rel="noopener noreferrer"
                    role="menuitem"
                    className="w-full flex items-center gap-2 px-4 py-2 text-sm text-slate-300 hover:bg-white/10"
                  >
                    <ExternalLink className="w-4 h-4" /> User Manual
                  </a>
            ...
            <button
              onClick={onSignIn}
              className="px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold transition-colors shadow-lg shadow-blue-500/20"
            >
              Sign In
            </button>
          )}
