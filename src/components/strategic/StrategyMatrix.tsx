...
const quadrantConfig = {
  SO: { label: 'SO Strategies', subtitle: 'Strengths + Opportunities', description: 'Use strengths to capitalize on opportunities', color: 'emerald', bgGradient: 'from-emerald-500 to-teal-600', lightBg: 'bg-emerald-50', border: 'border-emerald-200', textColor: 'text-emerald-700', icons: [Shield, Lightbulb], leverageLevels: [7, 4, 8], leverageRationale: 'Amplify reinforcing growth loops & self-organizing capabilities' },
  ST: { label: 'ST Strategies', subtitle: 'Strengths + Threats', description: 'Use strengths to mitigate threats', color: 'blue', bgGradient: 'from-blue-500 to-indigo-600', lightBg: 'bg-blue-50', border: 'border-blue-200', textColor: 'text-blue-700', icons: [Shield, Zap], leverageLevels: [8, 5, 6], leverageRationale: 'Strengthen balancing loops, governance rules & information access' },
  WO: { label: 'WO Strategies', subtitle: 'Weaknesses + Opportunities', description: 'Overcome weaknesses by pursuing opportunities', color: 'purple', bgGradient: 'from-purple-500 to-violet-600', lightBg: 'bg-purple-50', border: 'border-purple-200', textColor: 'text-purple-700', icons: [AlertCircle, Lightbulb], leverageLevels: [3, 6, 9], leverageRationale: 'Redefine goals, expose information gaps & reduce structural delays' },
  WT: { label: 'WT Strategies', subtitle: 'Weaknesses + Threats', description: 'Minimize weaknesses and avoid threats', color: 'amber', bgGradient: 'from-amber-500 to-orange-600', lightBg: 'bg-amber-50', border: 'border-amber-200', textColor: 'text-amber-700', icons: [AlertCircle, Zap], leverageLevels: [2, 5, 10], leverageRationale: 'Challenge paradigms, restructure stock-flow & establish defensive rules' },
};
...
const MEADOWS_LEVEL_COLORS: Record<string, string> = { high: 'bg-violet-600 text-white', mid: 'bg-red-500 text-white', feedback: 'bg-amber-500 text-white', params: 'bg-slate-400 text-white' };
  ...
  return (
    <TooltipProvider><Tooltip><TooltipTrigger asChild>
      <button onClick={() => onSelect(value)} className="transition-all transform hover:scale-110 active:scale-95">
        {Icon === Star ? <Star className={cn('w-4 h-4 transition-colors', value <= selectedValue ? 'text-amber-400 fill-current' : 'text-slate-300')} /> : <div className={cn('w-5 h-5 rounded-full border-2 flex items-center justify-center', value <= selectedValue ? 'border-cyan-500 bg-cyan-500' : 'border-slate-300 hover:border-cyan-300')}>{value <= selectedValue && <Check className="w-3 h-3 text-white" />}</div>}
      </button>
    </TooltipTrigger><TooltipContent side="top" className="max-w-[200px] z-50"><p className="font-semibold text-cyan-700">{type} Level {value}</p><p className="text-xs text-slate-600">{type === 'priority' ? description.priority : description.feasibility}</p></TooltipContent></Tooltip></TooltipProvider>
  );
};
...
const LeverageBadge: React.FC<{ lp: LeveragePoint }> = ({ lp }) => {
  ...
  return (
    ...
    </TooltipTrigger><TooltipContent side="bottom" className="max-w-[260px] z-50 space-y-1"><p className="font-semibold text-violet-700">[L{lp.leverageLevel}] {lp.meadowsName}</p><p className="text-xs text-slate-600">{lp.intervention}</p></TooltipContent></Tooltip></TooltipProvider>
  );
};
...
const StrategyCard: React.FC<{ option: StrategicOption; config: typeof quadrantConfig.SO; onUpdate: (updates: Partial<StrategicOption>) => void; onRemove: () => void; }> = ({ option, config, onUpdate, onRemove }) => {
  ...
  return (
    <div className={cn(`${config.lightBg} ${config.border} border rounded-xl p-4 group hover:shadow-md transition-all`, option.selected && 'ring-2 ring-cyan-500 ring-offset-2')}>
      {isEditing ? (
        <div className="space-y-3">
          <input type="text" value={editTitle} autoFocus onChange={e => setEditTitle(e.target.value)} className="w-full px-3 py-2 text-sm font-medium border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500" />
          <textarea value={editDesc} rows={3} onChange={e => setEditDesc(e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 resize-none" />
          <div className="flex justify-end gap-2">
            <button onClick={() => { setEditTitle(option.title); setEditDesc(option.description); setIsEditing(false); }} className="px-3 py-1.5 text-sm text-slate-600 rounded-lg">Cancel</button>
            <button onClick={handleSave} className="px-3 py-1.5 text-sm bg-cyan-500 text-white rounded-lg">Save</button>
          </div>
        </div>
      ) : (
        <>
          <div className="flex items-start justify-between gap-2 mb-2">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <button onClick={() => onUpdate({ selected: !option.selected })} className={cn('w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0', option.selected ? 'bg-cyan-500 border-cyan-500 text-white' : 'border-slate-300 hover:border-cyan-400')}>{option.selected && <Check className="w-3 h-3" />}</button>
              <h4 className="font-semibold text-slate-800 truncate">{option.title}</h4>
            </div>
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => setIsEditing(true)} className="p-1.5 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100"><Edit2 className="w-3.5 h-3.5" /></button>
              <button onClick={onRemove} className="p-1.5 text-red-400 hover:text-red-600 rounded-full hover:bg-red-50"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-200/50 flex-wrap gap-2">
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-1.5">
                ...
                <span className={cn('text-xs font-semibold w-5 text-center', (option.priorityScore || 3) >= 4 ? 'text-amber-600' : 'text-slate-500')}>{option.priorityScore || 3}</span>
              </div>
              <div className="flex items-center gap-1.5">
                ...
                <span className={cn('text-xs font-semibold w-5 text-center', (option.feasibilityScore || 3) >= 4 ? 'text-cyan-600' : 'text-slate-500')}>{option.feasibilityScore || 3}</span>
              </div>
            </div>
                    ...
                    {lps.map((lp, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-start gap-2 p-2 rounded-lg bg-white/70 border border-violet-100">
                          ...
                          <div className="flex-1 min-w-0"><p className="text-[11px] font-semibold text-slate-700 truncate">{lp.meadowsName}</p><p className="text-[10px] text-slate-500 leading-relaxed line-clamp-2">{lp.intervention}</p></div>
                        </div>
                      );
                    })}
                  </div>
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="text-sm text-slate-600"><span className="font-medium text-cyan-600">{selectedCount}</span> selected</div>
            <button onClick={handleGenerateStrategies} disabled={isGenerating || !canGenerate} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-500 to-violet-600 text-white rounded-xl text-sm font-medium hover:shadow-lg disabled:opacity-50 transition-all">
        ...
        {!canGenerate && <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3"><AlertCircle className="w-5 h-5 text-amber-600" /><div><p className="text-sm font-medium text-amber-800">Complete SWOT Analysis First</p><p className="text-xs text-amber-600">Current: {swotCounts.strengths}S, {swotCounts.weaknesses}W, {swotCounts.opportunities}O, {swotCounts.threats}T</p></div></div>}
        {generationError && <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3"><AlertTriangle className="w-5 h-5 text-red-600" /><p className="text-sm font-medium text-red-800">{generationError}</p></div>}
                ...
                <div className="p-4 space-y-3 max-h-96 overflow-y-auto">
                  ...
                  {newStrategy.quadrant === type ? (
                    <div className={cn(`${config.lightBg} ${config.border} border rounded-xl p-4 space-y-3`)}>
                      <input type="text" autoFocus value={newStrategy.title} onChange={e => setNewStrategy(p => ({ ...p, title: e.target.value }))} className="w-full px-3 py-2 text-sm font-medium border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500" placeholder="Strategy title" />
                      <textarea value={newStrategy.description} rows={2} onChange={e => setNewStrategy(p => ({ ...p, description: e.target.value }))} className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 resize-none" placeholder="Strategy description" />
                      <div className="flex justify-end gap-2">
                        <button onClick={() => setNewStrategy({ quadrant: null, title: '', description: '' })} className="px-3 py-1.5 text-sm text-slate-600 rounded-lg">Cancel</button>
                        <button onClick={handleAddStrategy} disabled={!newStrategy.title.trim()} className="px-3 py-1.5 text-sm bg-cyan-500 text-white rounded-lg disabled:opacity-50">Add Strategy</button>
                      </div>
                    </div>
                  ) : (
                    <button onClick={() => setNewStrategy({ quadrant: type, title: '', description: '' })} className={cn('w-full py-3 border-2 border-dashed rounded-xl text-sm font-medium text-slate-500 hover:text-slate-700 hover:border-slate-400 transition-colors flex items-center justify-center gap-2', config.lightBg)}><Plus className="w-4 h-4" /> Add {type} Strategy</button>
                  )}
                </div>
              </div>
            );
        ...
        {selectedCount > 0 && (
          <div className="bg-cyan-50 border border-cyan-200 rounded-xl p-6">
            <h3 className="font-semibold text-cyan-800 mb-4 flex items-center gap-2"><Check className="w-5 h-5" /> Selected Strategic Options ({selectedCount})</h3>
              ...
              {(plan.strategicOptions || []).filter(opt => opt.selected).sort((a, b) => ((b.priorityScore || 3) + (b.feasibilityScore || 3)) - ((a.priorityScore || 3) + (a.feasibilityScore || 3))).map(opt => {
                ...
                return (
                  <div key={opt.id} className="bg-white rounded-lg p-3 border border-cyan-200 flex items-start gap-3 hover:shadow-sm transition-shadow">
                    <span className={cn('px-2 py-1 rounded text-xs font-bold text-white shrink-0', opt.optionType === 'SO' ? 'bg-emerald-500' : opt.optionType === 'ST' ? 'bg-blue-500' : opt.optionType === 'WO' ? 'bg-purple-500' : 'bg-amber-500')}>{opt.optionType}</span>
                    <div className="flex-1 min-w-0"><p className="font-medium text-slate-800 text-sm truncate">{opt.title}</p><p className={cn('text-xs font-semibold', tier.color)}>Score: {total}/10</p></div>
                  </div>
                );
              })}
            </div>
