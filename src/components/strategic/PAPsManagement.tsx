...
const statusConfig = {
  planned: { label: 'Planned', bgColor: 'bg-slate-100 dark:bg-slate-800', textColor: 'text-slate-600 dark:text-slate-400', borderColor: 'border-slate-200 dark:border-slate-700' },
  ...
  cancelled: { label: 'Cancelled', bgColor: 'bg-slate-200', textColor: 'text-slate-500 dark:text-slate-400', borderColor: 'border-slate-300 dark:border-slate-600' },
};
    ...
    return (
      <div className="bg-white dark:bg-slate-800/60 rounded-xl border-2 border-cyan-400 p-4 space-y-4 shadow-xl ring-4 ring-cyan-50">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Spent ($)</label>
            <input
              type="number"
          ...
          <div className="col-span-2">
            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Linked Objective</label>
            <select
        ...
        <div className="flex justify-end gap-2 pt-2">
          <button onClick={() => setIsEditing(false)} className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-lg">
            <X className="w-4 h-4" /> Cancel
          </button>
          <button onClick={handleSave} className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium bg-cyan-600 text-white hover:bg-cyan-700 rounded-lg">
    ...
    <div className="bg-white dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 p-4 hover:shadow-xl hover:border-cyan-200 transition-all group relative overflow-hidden flex flex-col h-full">
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-1">
           <button onClick={() => setIsEditing(true)} className="p-1.5 text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-all opacity-0 group-hover:opacity-100">
            <Edit2 className="w-3.5 h-3.5" />
          </button>
          <button onClick={onRemove} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all opacity-0 group-hover:opacity-100">
      ...
      <div className="flex-grow">
        ...
        {pap.description && (
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 line-clamp-2 italic">
            "{pap.description}"
          </p>
        )}
        ...
        <div className="space-y-4 mb-4">
          {linkedObjective && (
            <div className="flex items-start gap-2 p-2 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-100 dark:border-slate-700/60">
              <Target className="w-3.5 h-3.5 text-slate-400 mt-0.5" />
              <div className="flex-1">
                <p className="text-[10px] uppercase font-bold text-slate-400 leading-none mb-1">Aligned Objective</p>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-end">
              <div className="flex items-center gap-1">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">Budget Utilization</span>
              </div>
      ...
      <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-700/60">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-[10px] font-bold text-slate-600 dark:text-slate-400">
              {pap.ownerName ? pap.ownerName.split(' ').map(n => n[0]).join('') : <User className="w-3 h-3" />}
            </div>
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase leading-none mb-0.5">Owner</p>
              <p className="text-xs font-medium text-slate-700 dark:text-slate-200">{pap.ownerName || pap.owner || 'Unassigned'}</p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between text-[10px] font-medium text-slate-400 bg-slate-50 dark:bg-slate-900 p-2 rounded-lg">
          <div className="flex items-center gap-1">
    ...
    <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-200 dark:border-slate-700">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Execute strategic goals through targeted programs, activities, and projects.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative group">
            <Filter className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-cyan-500" />
            <input 
              type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-800/60 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase">Financial Scope</span>
          </div>
          <h3 className="text-2xl font-black text-slate-900">${(stats.totalBudget / 1000).toFixed(1)}k</h3>
          <p className="text-xs text-slate-500 font-medium">Total Portfolio Budget</p>
        </div>
        
        <div className="bg-white dark:bg-slate-800/60 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-emerald-50 rounded-lg text-emerald-600"><TrendingUp className="w-5 h-5" /></div>
            <span className="text-[10px] font-bold text-slate-400 uppercase">Expenditure</span>
          </div>
          ...
          <div className="flex items-center gap-2 mt-1">
             ...
             <span className="text-[10px] font-bold text-slate-500">{stats.utilization.toFixed(0)}%</span>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800/60 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <p className="text-xs text-slate-500 font-medium">{plan.paps.length} Total Registered PAPs</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800/60 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <p className="text-xs text-slate-500 font-medium">{stats.inProgress} Currently In Progress</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between bg-white dark:bg-slate-800/60 p-3 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-slate-400 uppercase ml-2">Filter By:</span>
            <select 
              value={filterType}
              onChange={(e) => setFilterType(e.target.value)}
              className="text-sm font-semibold text-slate-700 bg-slate-50 dark:bg-slate-900 px-3 py-1.5 rounded-lg border-none outline-none focus:ring-2 focus:ring-cyan-500"
            >
        ...
        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
          <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-white dark:bg-slate-800/60 shadow-sm text-cyan-600' : 'text-slate-400 hover:text-slate-600'}`}>
            <LayoutGrid className="w-4 h-4" />
          </button>
          <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-white dark:bg-slate-800/60 shadow-sm text-cyan-600' : 'text-slate-400 hover:text-slate-600'}`}>
      ...
      {isAdding && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleAddSubmit} className="bg-white dark:bg-slate-800/60 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 bg-slate-900 text-white flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold">Initiate New PAP</h2>
                <p className="text-slate-400 text-sm">Define a new program, activity, or project for execution.</p>
              </div>
            ...
            <div className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">PAP Name</label>
                  <input required autoFocus placeholder="e.g., Annual Customer Satisfaction Survey" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-cyan-500 outline-none" value={newPAP.name} onChange={(e) => setNewPAP(p => ({ ...p, name: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Classification</label>
                  <select className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none" value={newPAP.papType} onChange={(e) => setNewPAP(p => ({ ...p, papType: e.target.value as any }))}>
                    <option value="program">Program</option>
                    <option value="activity">Activity</option>
                    <option value="project">Project</option>
                  </select>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Project Owner</label>
                  <input required placeholder="Full Name" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-cyan-500 outline-none" value={newPAP.ownerName} onChange={(e) => setNewPAP(p => ({ ...p, ownerName: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Start Date</label>
                  <input type="date" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none" value={newPAP.startDate} onChange={(e) => setNewPAP(p => ({ ...p, startDate: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">End Date</label>
                  <input type="date" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none" value={newPAP.endDate} onChange={(e) => setNewPAP(p => ({ ...p, endDate: e.target.value }))} />
                </div>
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Initial Budget ($)</label>
                  <div className="relative">
                    <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    <input type="number" placeholder="0.00" className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-cyan-500 outline-none" value={newPAP.budget} onChange={(e) => setNewPAP(p => ({ ...p, budget: parseFloat(e.target.value) || 0 }))} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-900 flex justify-end gap-3 border-t border-slate-200 dark:border-slate-700">
              <button type="button" onClick={() => setIsAdding(false)} className="px-6 py-2.5 font-bold text-slate-500 hover:text-slate-700 transition-colors">Cancel</button>
              <button type="submit" disabled={!newPAP.name.trim()} className="px-8 py-2.5 bg-cyan-600 text-white font-bold rounded-xl hover:bg-cyan-700 disabled:opacity-50 shadow-lg shadow-cyan-600/20 transition-all">Confirm Creation</button>
            </div>
          </form>
        </div>
      )}
      ...
      {filteredPAPs.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-32 bg-slate-50 dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-3xl text-center px-6">
          ...
          <p className="text-slate-500 max-w-sm">We couldn't find any PAPs matching your current filters. Try adjusting your search or add a new entry.</p>
        </div>
      )}
    </div>
  );
